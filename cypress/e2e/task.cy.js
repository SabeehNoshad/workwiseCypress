...
describe('Task module Type General', () => {
  it('should check for the correct url', () => {
    ...
    cy.get(':nth-child(1) > .ant-collapse-item > .ant-collapse-content > .ant-collapse-content-box > [role="presentation"] > :nth-child(14) > a').click()
    cy.url().should('eq','https://www.workw.com/tasks')
  });
  it('should not create without mandatory fields', () => {
    login(Cypress.env('adminemail'),Cypress.env('adminpassword'));
    cy.get(':nth-child(1) > .ant-collapse-item > .ant-collapse-content > .ant-collapse-content-box > [role="presentation"] > :nth-child(14) > a').scrollIntoView().should('be.visible').click()
...
describe('Task module Type project', () => {
  ...
  it('should not create without mandatory fields', () => {
    ...
    it.only('should create task with only mandatory fields', () => {
      login(Cypress.env('adminemail'),Cypress.env('adminpassword'));
    cy.get(':nth-child(1) > .ant-collapse-item > .ant-collapse-content > .ant-collapse-content-box > [role="presentation"] > :nth-child(14) > a').scrollIntoView().should('be.visible').click()
    cy.get('.buttons > :nth-child(2) > .ant-btn').click();
